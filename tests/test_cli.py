...
from gatk_sv_profile.cli import AnalysisConfig, build_parser, main
...
def test_cli_preprocess_builds_config(tmp_path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr("gatk_sv_profile.cli.run_preprocess", fake_run_preprocess)
...
def test_cli_analyze_runs_selected_modules_and_skips_unmet_requirements(make_vcf, tmp_path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr("gatk_sv_profile.cli.ALL_MODULES", [DemoModule, SharedDemoModule, PedDemoModule])
...
def test_cli_run_executes_preprocess_then_analyze(tmp_path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr("gatk_sv_profile.cli.ALL_MODULES", [DemoModule])
    monkeypatch.setattr("gatk_sv_profile.cli.run_preprocess", fake_run_preprocess)
    monkeypatch.setattr("gatk_sv_profile.cli.aggregate", fake_aggregate)
